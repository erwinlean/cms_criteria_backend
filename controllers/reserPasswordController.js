...
module.exports = {
  ...
  passwordReset: async function (req, res, next) {
    try {
      ...
      const resetLink = `http://localhost:8080/api/reset/password/${resetToken}?email=${email}`;  
  ...
  redirect: async function (req, res, next) {
    ...
    const redirectURL = `http://127.0.0.1:5500/password-reset.html?token=${token}&email=${email}`;
